...
public class PolygonTool extends SizeableTool{
    ...
    {
        //TODO: figure out how to make irregular/regular work with live preview
        /*screen.getPreviewCanvas().setOnMousePressed(event -> {
            super.install(screen, colorPicker);

            if (isIrregular)
                drawIrregularPolygon(event, screen.getPreviewGraphics());
            else
                drawRegularPolygon(event, screen.getPreviewGraphics());
        });
        ...
        screen.getDrawingCanvas().toFront(); //TEMP
        screen.getDrawingCanvas().setOnMouseClicked(event -> {
            super.install(screen, colorPicker);

            if (isIrregular)
                drawIrregularPolygon(event, screen.getDrawingGraphics());
            else
                drawRegularPolygon(event, screen.getDrawingGraphics());
        });
    }
    ...
    public void drawRegularPolygon(MouseEvent event, GraphicsContext graphics)
    {
        ...
        else
        {
            ...
            //select the smallest radii from the rectangle created by the clicks
