...
public class ShapeTool extends SizeableTool{
     ...
     */
    public ShapeTool() {shape = Shape.RECTANGLE;} //default shape is rectangle
     ...
     */
    public ShapeTool(Shape shape) {this.shape = shape;}

    public void install(Screen screen, ColorPicker colorPicker)
    {
        //TODO: think about shoving the logic into a class that calculates/returns points, and break up main class into 4 small classes
        screen.getDrawingCanvas().setOnMouseClicked(event -> {
            ...
            else
            {
                ...
                if(shape == Shape.RECTANGLE || shape == Shape.SQUARE)
                    screen.getDrawingGraphics().strokeRect(firstPoint.getX(), firstPoint.getY(), width, height);
                else
                    screen.getDrawingGraphics().strokeOval(firstPoint.getX(), firstPoint.getY(), width, height);
